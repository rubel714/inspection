...
import InspectionCompletionReport from "./InspectionCompletionReport";

const Index = (props) => {
  const { path } = useRouteMatch();
  const menukey = "inspectioncompletionreport"; // this is in t_menu table
      ...
      <div >
        ...
        <Switch>
          <Route
            path={`${path}/`}
            render={(props) => (
              <InspectionCompletionReport {...props} openNoticeModal={openNoticeModal} />
            )}
          ></Route>
        </Switch>
        <Notification
