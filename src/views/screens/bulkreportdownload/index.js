...
import BulkReportDownload from "./BulkReportDownload";

const Index = (props) => {
  const { path } = useRouteMatch();
  const menukey = "bulkreportdownload"; // this is in t_menu table
      ...
      <div >
        ...
        <Switch>
          <Route
            path={`${path}/`}
            render={(props) => (
              <BulkReportDownload {...props} openNoticeModal={openNoticeModal} />
            )}
          ></Route>
        </Switch>
        <Notification
